...
import { Plus, Trash2, Edit, Calendar, Clock, Filter } from 'lucide-react'
...
const Seizures = () => {
  ...
  const getDateRange = () => {
    switch (filterMode) {
      case 'recent':
        // Get last 10 years worth to ensure we get 20+ seizures
  ...
  // Fetch seizures based on filter mode
  const { data: allSeizures = [], isLoading } = useQuery({
    queryKey: ['seizures', filterMode, selectedYear, selectedMonth, selectedDate],
    queryFn: () => {
      const { start, end } = getDateRange()
      return getSeizures(start, end).then(res => res.data)
    },
  })

  // For 'recent' mode, limit to last 20 seizures
  const seizures = filterMode === 'recent'
    ? allSeizures.slice(0, 20)
    : allSeizures
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-gray-700">Filter seizures by:</span>
        </div>
        ...
        {filterMode === 'recent' && (
          <p className="text-sm text-gray-500">
            Showing the most recent 20 seizure entries
          </p>
        )}
      </div>
